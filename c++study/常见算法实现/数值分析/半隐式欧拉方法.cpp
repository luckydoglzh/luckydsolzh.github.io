 ...
 * \brief 使用 [半隐式欧拉方法](https://en.wikipedia.org/wiki/Semi-implicit_Euler_method) 求解多变量一阶 [常微分方程 (ODEs)](https://en.wikipedia.org/wiki/Ordinary_differential_equation)
 *
 * \details
 * 求解的常微分方程为：
 * \f{eqnarray*}{
 ...
 * v(x) &=& -\sin(x)\\
 * \f}
 * 计算结果会存储到文本文件 `semi_implicit_euler.csv` 中，精确解的结果存储在 `exact.csv` 文件中，以便对比。
 * <img
...
void problem(const double &x, std::valarray<double> *y,
    ...
    const double omega = 1.F;             // 问题中的常数 ω
    dy[0][0] = y[0][1];                   // x 的导数
    dy[0][1] = -omega * omega * y[0][0];  // y 的导数
}
 ...
 */
/**
 * @brief 使用半隐式欧拉方法计算下一个近似值。公式为：@f[y_{n+1}=y_n + dx\cdot f\left(x_n,y_n\right)@f]
 * @param[in] 		dx	步长
 * @param[in] 	    x	自变量值 \f$x_n\f$
 * @param[in,out] 	y	因变量值 \f$y_n\f$
 * @param[in,out]	dy	计算 \f$f\left(x_n,y_n\right)\f$
 */
void semi_implicit_euler_step(const double dx, const double &x,
                              std::valarray<double> *y,
                              std::valarray<double> *dy) {
    problem(x, y, dy);         // 更新 dy（因变量的导数）
    y[0][0] += dx * dy[0][0];  // 更新 y0（自变量的值）
    problem(x, y, dy);         // 再次更新 dy（因变量的导数）

    dy[0][0] = 0.f;      // 忽略 y0 的影响
    y[0] += dy[0] * dx;  // 使用新的 dy 更新剩余的因变量
}

/**
 * @brief 在给定的范围内使用半隐式欧拉方法计算近似解。
 * @param[in] 		dx  	步长
 * @param[in]   	x0  	自变量初值
...
double semi_implicit_euler(double dx, double x0, double x_max,
    ...
    std::valarray<double> dy = y[0];  // 因变量的导数
    ...
    do {  // 遍历每个自变量的步长
        ...
        semi_implicit_euler_step(dx, x, y, &dy);  // 执行半隐式欧拉法
        x += dx;                            // 更新自变量
