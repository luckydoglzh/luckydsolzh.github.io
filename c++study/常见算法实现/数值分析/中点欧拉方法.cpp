 ...
 * \brief 使用 [中点欧拉方法](https://en.wikipedia.org/wiki/Midpoint_method) 求解多变量一阶 [常微分方程 (ODEs)](https://en.wikipedia.org/wiki/Ordinary_differential_equation)
 *
 * \details
 * 求解的常微分方程为：
 * \f{eqnarray*}{
 ...
 * v(x) &=& -\sin(x)\\
 * \f}
 * 计算结果会存储到文本文件 `midpoint_euler.csv` 中，精确解的结果存储在 `exact.csv` 文件中，以便对比。
 * <img
 ...
 */
/**
 * @brief 使用中点欧拉方法计算下一个近似值。公式为：@f[y_{n+1} = y_n + dx\, f\left(x_n + \frac{1}{2} dx, y_n + \frac{1}{2} dx\, f\left(x_n, y_n\right)\right)@f]
 * @param[in] 		dx	步长
...
void midpoint_euler_step(const double dx, const double &x,
    ...
    y[0] += dy[0] * dx;  // 使用中点欧拉方法更新 y
}

/**
 * @brief 在给定的范围内使用中点欧拉方法计算近似解。
 * @param[in] 		dx  	步长
 * @param[in]   	x0  	自变量初值
...
double midpoint_euler(double dx, double x0, double x_max,
    ...
    do {  // 遍历每个自变量的步长
        ...
        midpoint_euler_step(dx, x, y, &dy);  // 执行中点欧拉方法
        x += dx;                            // 更新自变量
